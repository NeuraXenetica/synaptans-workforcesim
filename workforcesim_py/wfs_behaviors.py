...
"""
This module handles the logic that simulates the *actual* behaviors 
performed by workers during each day of the simulated time period. These
behaviors reflect the “reality” of the daily productivity and 
interpersonal interactions of workers in the factory.
"""
...
def configure_behavs_act_df():
    """
    Configures behavs_act_df -- the master DataFrame that contains an 
    entry for every actual behavior performed by workers (including a 
    worker's Absence, which is itself a sort of behavior) -- by creating
    initial columns needed for adding behavior entries.
    """

    cfg.behavs_act_df = pd.DataFrame()

    # Create the DataFrame's initial columns.
    ...
    cfg.behavs_act_df["Behavior Type"] = ""
    cfg.behavs_act_df["Behavior Comptype"] = ""
    cfg.behavs_act_df["Behavior Nature"] = ""
...
def print_modified_probabilities_of_a_person(
    ...
    """
    Prints a person's stats after the daily update of their modifiers.

    PARAMETERS
    ----------
    person_u
        Person object for the person whose stats should be printed
    """

    print(
        "person_u.prob_modified_presence: ", 
        person_u.prob_modified_presence
        )
    print(
        "person_u.prob_modified_idea: ", 
        person_u.prob_modified_idea
        )
    print(
        "person_u.prob_modified_lapse: ", 
        person_u.prob_modified_lapse
        )
    ...
    print(
        "person_u.prob_modified_teamwork: ", 
        person_u.prob_modified_teamwork
        )
    print(
        "person_u.prob_modified_disruption: ", 
        person_u.prob_modified_disruption
        )
    print(
        "person_u.prob_modified_sacrifice: ", 
        person_u.prob_modified_sacrifice
        )
    print(
        "person_u.prob_modified_sabotage: ", 
        person_u.prob_modified_sabotage
        )
    print("person_u.prob_modified_recording_accurately: ", 
        person_u.prob_modified_recording_accurately
        )
...
def add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
    ...
    *in the moment of the behavior*, thereby creating a lasting
    ...
    ----------
    person_object_u
        Person object for the person who performed the behavior
    behavior_type_u : str
        Behavior type
    behavior_subtype_u : str
        Behavior subtype
    behavior_nature_u : str
        Behavior nature
    behavior_comptype_u : str
        Behavior comptype
    eff_score_u
        The actual Efficacy (if relevant) demonstrated in the behavior
    include_record_u
        Whether to include a record (e.g., for a Separation or 
        Onboarding event)
    """

    # NOTE: This function originally immediately added each new behavior
    # to behavs_act_df as a concatenated row, but that approach
    # generated exponential copying effects tied to the number of days 
    # in the sim. Now all of the rows are temporarily stored and all 
    # added to the DataFrame in a single step at the end.
    ...
    # Calculate the values of fields related to the subject's 
    # supervisor. This is only relevant if a person *has* a supervisor 
    # (e.g., if he isn't the factory's Production Director).
    try:
    ...
    behav_to_add_df = pd.DataFrame({
        ...
        "Sub Same-Sex Colleagues Prtn": \
            [person_object_u.colleagues_of_same_sex_prtn],
        ...
        "Actual Efficacy (SD)": [eff_score_u],
        "Record Type": None,
    ...
    # ------------------------------------------------------------------
    # Include a record for workers' Separation (Resignation or 
    # Termination) and Onboarding events. These events are always 
    # recorded by managers with 100% accuracy, and the record is added 
    # to the DF at the same time as the underlying behavior, as part of 
    # a unified event.
    #
    # (This is necessary because Separation and Onboarding is handled) 
    # at a different time of day, outside of the normal daily event 
    # cycle.)
    # ------------------------------------------------------------------
    if include_record_u is True:
        if (behav_to_add_df["Behavior Type"].values[0] == "Separation") \
            ...
            # The lack of a Note will be represented by an empty 
            # string ("") rather than a None value.
            behav_to_add_df["Note"].values[0] = ""

            # If the event was a Termination (and not a Resignation), 
            # then the event's type and cause should be noted in the 
            # *record* component of the row (since it was the supervisor
            # who initiated that event, not the subordinate) -- not in 
            # the behavior component, as is the case with a Resignation 
            # event.
            # 
            # Reflect this by (1) changing the "Behavior Type" and 
            # "Behavior Comptype" to None values, and (2) moving the 
            # description of the cause from the "Behavior Nature" column
            # to the "Record Nature" column and filling the "Behavior 
            # Nature" field with a None value.
            #
            # Here, the event will thus have only a record component, 
            # not a behavior component.
            if behav_to_add_df["Behavior Comptype"].values[0] \
                    == "Termination":
...
def simulate_one_day_of_behaviors():
    ...
    # Calculate the mean of all actual Eff values generated in the org 
    # to date. Only include the values from the Eff-value dictionaries 
    # up through the current date of the simulation (the later values 
    # will be None).
    list_of_all_actual_eff_values = []
    for p in cfg.persons:
        list_of_all_actual_eff_values.extend(
            list(cfg.persons[p].dict_days_with_actual_eff_values.values())[0:(
                cfg.day_of_sim_iter \
    ...
    try:
        # Remove all None values from the list of actual Efficacy 
        # values.
        list_of_all_actual_eff_values \
            = [v for v in list_of_all_actual_eff_values if v is not None]
        cfg.org_actual_eff_values_mean = \
            statistics.mean(list_of_all_actual_eff_values)
    # This is the case at the start of the first simulated day, when the
    # length of the list of generated values will have a length of 0.
    except:
    ...
    for p in cfg.persons:
        list_of_all_recorded_eff_values.extend(
            list(cfg.persons[p].dict_days_with_recorded_eff_values.values())[0:(
                cfg.day_of_sim_iter \
                    - cfg.day_of_sim_iter_for_first_simulated_day
                    )]
                    )
    try:
        # Remove all None values from the list of recorded Efficacy 
        # values.
        list_of_all_recorded_eff_values \
            = [v for v in list_of_all_recorded_eff_values if v is not None]
    ...
    print(
        "   cfg.org_actual_eff_values_mean at start of day: ", 
        cfg.org_actual_eff_values_mean
        )
    ...
    for p in cfg.persons:
        ...
        # --------------------------------------------------------------
        # Calculate the person's attendance status and generate a 
        # "Presence" or "Absence" behavior.
        # --------------------------------------------------------------

        # Every person has some probability of randomly generating an 
        # Absence. First, check to see if that occurs.

        # If the person's modified probability of a Presence is less 
        # than a random number from 0.00-1.00, the person is absent 
        # (i.e., performs an Absence behavior).
        ...
        # If the person *doesn't* generate a random Absence behavior, 
        # check to see if he generates one as a result of having had at 
        # least 1 Lapse and 1 Slip in the previous 4 days.
        else:
            lapses_num_previous_4_days = 0
            day_deltas = [-4, -3, -2, -1]
            days_to_search \
                = [cfg.day_of_sim_iter + delta for delta in day_deltas]
            for day in days_to_search:
                # Confirm that the sought day is within the simulated 
                # range.
            ...
            # If the person has at least 1 Lapse and 1 Slip in the 
            # previous 4 days...
            if (lapses_num_previous_4_days >= 1) \
                and (slips_num_previous_4_days >= 1):

                # ... he has a 90% chance of having an Absence today.
                if (random.uniform(0.0, 1.0) <= 0.9):
                    attendance_today = "absent"

                    # Add the person's "Absence" behavior to the 
                    # behaviors DF.
                    add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                        cfg.persons[p],
                        "Attendance",
                ...
                # If he succeeds at the 10% roll, then he is present 
                # today.
                else:
                    attendance_today = "present"

                    # Add the person's "Presence" behavior to the 
                    # behaviors DF.
                    add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                        cfg.persons[p],
                        "Attendance",
            ...
            # If he doesn't have at least 1 Lapse and 1 Slip in the 
            # previous 4 days, then he is present today.
            else:
                attendance_today = "present"
        ...
        # --------------------------------------------------------------
        # If the person is present, calculate whether (or how) he 
        # performs particular types of behaviors.
        # --------------------------------------------------------------
        if attendance_today == "present":
            ...
            eff_sco_today = round( float( (
                ...
                * 1 + ( np.random.normal(
                    loc=0, scale=cfg.BASE_MAX_EFFICACY_VARIABILITY, size=1) \
            ...
            # In this Person object's dictionary that stores the number 
            # of events of this type that have occurred on each day, add
            # the actual Efficacy value. (At dict creation, the values 
            # for all days are 0.0.)
            # Note! This presumes that a person can only generate one 
            # Efficacy behavior per day. The new value overwrites any 
            # existing value for the day.
            cfg.persons[p].dict_days_with_actual_eff_values[cfg.day_of_sim_iter] \
                = eff_sco_today
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate an Idea 
            # behavior...
            if cfg.persons[p].prob_modified_idea \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_GOOD)):

                # In this Person object's dictionary that stores the 
                # number of events of this type that have occurred on 
                # each day, increment the value for this day (as the 
                # key) by 1. (At dict creation, the values for all days 
                # are 0.)
                cfg.persons[p].dict_days_with_num_of_idea_behaviors[
                    cfg.day_of_sim_iter
                    ] += 1
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate a Lapse 
            # behavior...
            if cfg.persons[p].prob_modified_lapse \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_POOR)):
                ...
                # key) by 1. (At dict creation, the values for all days 
                # are 0.)
                cfg.persons[p].dict_days_with_num_of_lapse_behaviors[
                    cfg.day_of_sim_iter
                    ] += 1
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate a Feat 
            # behavior...
            if cfg.persons[p].prob_modified_feat \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_GOOD)):
            ...
            # ----------------------------------------------------------
            # Every person has some probability of randomly generating a
            # Slip. First, check to see if that occurs.
            # If the person meets the threshold to generate a random 
            # Slip behavior...
            if cfg.persons[p].prob_modified_slip \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_POOR)):
                ...
                # key) by 1. (At dict creation, the values for all days 
                # are 0.)
                cfg.persons[p].dict_days_with_num_of_slip_behaviors[
                    cfg.day_of_sim_iter
                    ] += 1
            ...
            # If the person *doesn't* generate a random Slip behavior, 
            # check to see if he generates one as a result of having had
            # 2 or more Lapses in the previous 4 days.
            else:
                lapses_num_previous_4_days = 0
                day_deltas = [-4, -3, -2, -1]
                days_to_search = \
                    [cfg.day_of_sim_iter + delta for delta in day_deltas]
                for day in days_to_search:
                    # Confirm that the sought day is within the 
                    # simulated range.
                    if day in cfg.persons[p].dict_days_with_num_of_lapse_behaviors:
                        lapses_num_previous_4_days += \
                            cfg.persons[p].dict_days_with_num_of_lapse_behaviors[day]

                # If the person has 2 or more Lapses in the previous 
                # 4 days...
                if lapses_num_previous_4_days >= 2:
                    ...
                    if (random.uniform(0.0, 1.0) <= 0.9):
                        ...
                        # In this Person object's dictionary that stores
                        # the number of events of this type that have 
                        # occurred on each day, increment the value for 
                        # this day (as the key) by 1. (At dict creation,
                        # the values for all days are 0.)
                        cfg.persons[p].dict_days_with_num_of_slip_behaviors[
                            cfg.day_of_sim_iter
                            ] += 1

                        # Add the person's "Slip" behavior to the 
                        # behaviors DF.
                        add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                            cfg.persons[p],
                            "Poor",
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate a Teamwork 
            # behavior...
            if cfg.persons[p].prob_modified_teamwork \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_GOOD)):
                ...
                # key) by 1. (At dict creation, the values for all days 
                # are 0.)
                cfg.persons[p].dict_days_with_num_of_teamwork_behaviors[
                    cfg.day_of_sim_iter
                    ] += 1

                # Add the person's "Teamwork" behavior to the 
                # behaviors DF.
                add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                    cfg.persons[p],
                    "Good",
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate a Disruption
            # behavior...
            if cfg.persons[p].prob_modified_disruption \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_POOR)):

                # Add the person's "Disruption" behavior to the 
                # behaviors DF.
                add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                    cfg.persons[p],
                    "Poor",
            ...
            # ----------------------------------------------------------
            # Every person has some probability of randomly generating a
            # Sacrifice. First, check to see if that occurs.
            # If the person meets the threshold to generate a Sacrifice 
            # behavior...
            if cfg.persons[p].prob_modified_sacrifice \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_GOOD)):

                # Add the person's "Sacrifice" behavior to the 
                # behaviors DF.
                add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                    cfg.persons[p],
                    "Good",
            ...
            # If the person *doesn't* generate a random Sacrifice 
            # behavior, check to see if he generates one as a result of 
            # having had 2 or more Teamworks in the previous 5 days.
            else:
                ...
                # If the person has 2 or more Teamworks in the previous 
                # 5 days...
                if teamworks_num_previous_5_days >= 2:

                    # ... he has an 80% chance of having a Sacrifice 
                    # today.
            ...
            # ----------------------------------------------------------
            # If the person meets the threshold to generate a Sabotage 
            # behavior...
            if cfg.persons[p].prob_modified_sabotage \
                >= (random.uniform(0.0, cfg.DEFENSE_ROLL_MAX_BEHAVIOR_POOR)):

                # Add the person's "Sabotage" behavior to the 
                # behaviors DF.
                add_one_behav_to_list_of_behavs_to_add_to_behavs_act_df(
                    cfg.persons[p],
                    "Poor",
    ...
    # ------------------------------------------------------------------
    # Concatenate behavs_act_df with the individual DFs for the day, 
    # each of which is a one-row DF containing info on a particular 
    # behavior. This can't be delayed further (to avoid exponential 
    # copying), because it's needed in order to now calculate managers' 
    # recordings for the given day.
    # ------------------------------------------------------------------

    # If len(cfg.behavs_act_df) == 0, this is the first day to have been 
    # simulated; simply concatenate all of this day's DFs to one 
    # another.
    if len(cfg.behavs_act_df) == 0:
        list_of_dfs_to_concatenate \
            = cfg.list_of_behavs_to_add_to_behavs_act_df

    # If len(cfg.behavs_act_df) != 0, then that DF already contains one 
    # or more days' worth of behaviors; concatenate all of this day's 
    # new DFs to that existing cfg.behavs_act_df.
    else:
        list_of_dfs_to_concatenate = [cfg.behavs_act_df]
        list_of_dfs_to_concatenate.extend(
            cfg.list_of_behavs_to_add_to_behavs_act_df
            )
...
def calculate_metrics_for_persons_in_retained_simulated_period():
    ...
    for p in cfg.persons:
        ...
        # Prepare a DataFrame containing only the behaviors for the 
        # given person.
        ...
        try:
            days_present = \
                behavs_act_df_this_pers["Behavior Comptype"].value_counts()["Presence"]
        except:
            days_present = 0

        # NOTE! These really shouldn't be thought of as "scores". 
        ...
        # NOTE! This will generate an error if there *aren't* any Good 
        # or Poor behaviors in the dataset -- which is possible, if the 
        # number of persons or number of days simulated is very small.
        try:
            good_count \
                = behavs_act_df_this_pers["Behavior Type"].value_counts()["Good"]
        except:
            good_count = 0
        try:
            poor_count \
                = behavs_act_df_this_pers["Behavior Type"].value_counts()["Poor"]
        except:
            poor_count = 0
...
def return_behavs_act_df_for_person_for_DpmN(
    version_of_behavs_act_df_to_use_u,
    person_u,
    DpmN_N_value_u,
    ):
    """
    ...
    ----------
    version_of_behavs_act_df_to_use_u
        The version of behavs_act_df to use
    person_u
        The Person object whose data should be sought
    DpmN_N_value_u
        Relative number of the earlier/later day to seek (-1, 1, etc.)
    """
...
def return_eff_modifier_for_impact_of_previous_recordings_on_bhv_of_person_today(
    ...
    ----------
    person_u
        The Person object whose behavior may be impacted
    """
    ...
    # Create a DF with just True Positive behaviors for the person in 
    # question.
    temp_df = utils.return_df_with_rows_filtered_to_one_val_in_col(
        cfg.behavs_act_df,
        "Sub ID",
    ...
    # ------------------------------------------------------------------
    # Get any Eff modifier component resulting from inaccurately 
    # recorded (False Negative) Idea, Feat, Teamwork, or Sacrifice 
    ...
    # Create a DF with just False Negative behaviors for the person in 
    # question.
    temp_df = utils.return_df_with_rows_filtered_to_one_val_in_col(
        cfg.behavs_act_df,
        "Sub ID",
...
def display_simple_behavior_statistics():
    """
    Displays basic statistics about the quantity of particular types of
    behaviors that were generated by persons in the focal period of the
    simulation.
    """
    ...
    print(
        "Good behaviors per person per day: ",
        behavs_good_num_per_pers_per_day
        )
    print("Number of Poor behaviors: ", behavs_poor_num)
    print(
        "Poor behaviors per person per day: ", 
        behavs_poor_num_per_pers_per_day
        )
